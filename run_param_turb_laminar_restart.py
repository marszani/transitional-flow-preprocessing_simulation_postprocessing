...
turb_intensity_vals = [0.0008]
eddy_vis_ratio_vals = [0.001]

for turb_intensity in turb_intensity_vals:
    for eddy_vis_ratio in eddy_vis_ratio_vals:
        ...
        for turbulenceModel in args.turbulenceModels:
            aeroOptions = {
                ...
                'turbResScale': [1e0, 1e-0, 1e-0, 1e-0], # [1e3, 1e-14, 1e-14, 1e-25],
                ...
                "L2Convergence": 1e-14,  
                "nCycles": 10000,
                'turbIntensityInf' : turb_intensity,  # range 0 : 3
                'eddyVisInfRatio' : eddy_vis_ratio,   # range 0.1 : 100
            ...
            for mach, reynolds, alpha in mach_reynolds_alpha_tuples:
                for gridFile in gridFiles:
                    # Create a unique output directory for each grid file, turbulence model, Mach, Reynolds, alpha, turb intensity and eddy vis ratio (only rank 0)
                    outputDir = os.path.join(
                        args.output, gridFile.split(".")[0], turbulenceModel,
                        f"M{mach}_Re{int(reynolds)}_alpha{alpha}_TI{turb_intensity}_EVR{eddy_vis_ratio}"
                    )
                    ...
                    restart_filename = f'{gridFile.split(".")[0]}_M{mach}_Re{int(reynolds)}_alpha{alpha}_TI{turb_intensity}_EVR{eddy_vis_ratio}_000_vol.cgns'
                    ...
                    if check_restart_file(restartFile):
                        ...
                        # Define AeroProblem
                        ap = AeroProblem(
                            name=f"{gridFile.split('.')[0]}_M{mach}_Re{int(reynolds)}_alpha{alpha}_TI{turb_intensity}_EVR{eddy_vis_ratio}",
                            mach=mach,
                            T=288,
                            reynolds=reynolds,
                            reynoldsLength = 1.0,
                            ...
                            chordRef=1.0,
                            evalFuncs=["cl", "cd","cmz"],
                          #  turbIntensityInf=turb_intensity,
                          #  eddyVisInfRatio=eddy_vis_ratio,
                        )
                        ap.alpha = alpha
                        ...
                        if args.task == "analysis":
                            ...
                            # Write results to a detailed text file based on simulation parameters
                            if comm.rank == 0:
                                summary_file = os.path.join(args.output, "analysis_summary.txt")
                                with open(summary_file, "a") as rf:
                                    rf.write(f"{gridFile.split('.')[0]}\t{turbulenceModel}\tMach={mach}\tRe={int(reynolds)}\tAlpha={alpha}\tTI={turb_intensity}\tEVR={eddy_vis_ratio}\t")
                                    rf.write(f"cl={funcs.get(f'{ap.name}_cl', 'nan')}\tcd={funcs.get(f'{ap.name}_cd', 'nan')}\tcmz={funcs.get(f'{ap.name}_cmz', 'nan')}\n")
                        ...
                        # Run ADflow in polar mode (sweeping AoA)
                            ...
                            for alpha in alphaList:
                                ap.name = f"{gridFile.split('.')[0]}_M{mach}_Re{int(reynolds)}_TI{turb_intensity}_EVR{eddy_vis_ratio}_{alpha:.2f}"
                                ap.alpha = alpha
                            ...
                            # Write polar results to a detailed summary file
                            summary_file = os.path.join(args.output, "polar_summary.txt")
                            with open(summary_file, "a") as rf:
                                for alpha, cl, cd in zip(alphaList, CLList, CDList):
                                    rf.write(f"{gridFile.split('.')[0]}\t{turbulenceModel}\tMach={mach}\tRe={int(reynolds)}\tAlpha={alpha:.2f}\tTI={turb_intensity}\tEVR={eddy_vis_ratio}\t")
                                    rf.write(f"cl={cl}\tcd={cd}\n")
