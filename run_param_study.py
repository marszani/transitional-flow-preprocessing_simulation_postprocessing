# parametric study of turbIntensityInf and eddyVisInfRatio  
# simulations of cases laminar, SST, LM-SST and saves convergence history
    ...
    for eddy_vis_ratio in eddy_vis_ratio_vals:
        ...
        for turbulenceModel in args.turbulenceModels:
            aeroOptions = {
                ...
                #'restartFile': 'sd7003_L2_M0.1_Re60000_alpha4_TI0.0003_EVR1.0_000_vol.cgns',
                'equationMode': 'steady',
                ...
                #'CFLCoarse': 1e-2,
                'nCyclesCoarse': 500,
                'L2ConvergenceCoarse': 0.001,

                
                "turbulenceModel": turbulenceModel if turbulenceModel != "laminar" else "Menter SST",
                'turbResScale': [1e0 * val for val in [1.0, 1.0, 1.0, 1.0]], # [1e3, 1e-14, 1e-14, 1e-25],
            ...
            for mach, reynolds, alpha in mach_reynolds_alpha_tuples:
                for gridFile in gridFiles:
                    # Create a unique output directory for each grid file, turbulence model, Mach, Reynolds, alpha, turb intensity and eddy viscosity ratio (only rank 0)
                    outputDir = os.path.join(args.output, gridFile.split(".")[0], turbulenceModel,
