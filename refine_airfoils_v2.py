...
for r, level in zip(refinements, levels):
    ...
    options = {
        ...
        # Marching grid parameters
        "N": nExtPts,
        "s0": s0,
        "nConstantStart": 30,         # 30 prism-like layers near the airfoil
        "marchDist": 100.0,            # Total extrusion distance

        # Mesh growth and smoothing
        "epsE": 1.0,
        "epsI": 2.0,
        "theta": 3.0,
        "volSmoothIter": 100,
        "coarsen": 1                  # No surface mesh coarsening; set 2 to coarsen
    }

    try:
        hyp = pyHyp(options=options)
        hyp.run()
        hyp.writeCGNS(os.path.join(output_dir, f"{base_filename}.cgns"))
