...
for case in case_names:
    ...
    print(f"Processing {langtry_path} and {menter_path}")
...
for z_value in next(iter(all_case_data.values()))["menter"].keys():
    for case, data in all_case_data.items():
        ...
        upper_min_xoc = 1e-17
        upper_max_xoc = 1-1e-17
        lower_min_xoc = 1e-17
        lower_max_xoc = 1-1e-17
        ...
        plt.plot(upper_surface_menter[:, 3], upper_surface_menter[:, variables_index["CoefPressure"]], label='Menter SST (Upper Surface)', color='b', marker='o', linestyle='None',markersize=1, linewidth=1)
        plt.plot(upper_surface_langtry[:, 3], upper_surface_langtry[:, variables_index["CoefPressure"]], label='Langtry Menter SST (Upper Surface)', color='r', marker='o', linestyle='None',markersize=1, linewidth=1)
        plt.plot(lower_surface_menter[:, 3], lower_surface_menter[:, variables_index["CoefPressure"]], label='Menter SST (Lower Surface)', color='b', marker='x', linestyle='None', markersize=1, linewidth=1)
        plt.plot(lower_surface_langtry[:, 3], lower_surface_langtry[:, variables_index["CoefPressure"]], label='Langtry Menter SST (Lower Surface)', color='r', marker='x', linestyle='None', markersize=1, linewidth=1)

        plt.title(rf'{sim_folder_name}: {var_name_label} vs $X/C$ | $M={mach:.2f}$, $Re={reynolds:.2e}$, $\alpha={alpha:.0f}^\circ$')
        plt.xlabel(r'$X/C$')
...
for z_value in next(iter(all_case_data.values()))["menter"].keys():
    for case, data in all_case_data.items():
        ...
        for var_name, col_idx in variables_index.items():
            ...
            plt.figure(figsize=(6, 6))
            plt.plot(upper_surface_menter[:, 3], var_data_menter_upper, label='Menter SST (Upper Surface)', color='b', marker='o', linestyle='None',markersize=1, linewidth=1)
            plt.plot(upper_surface_langtry[:, 3], var_data_langtry_upper, label='Langtry Menter SST (Upper Surface)', color='r', marker='o', linestyle='None',markersize=1, linewidth=1)
            plt.plot(lower_surface_menter[:, 3], var_data_menter_lower, label='Menter SST (Lower Surface)', color='b', marker='x', linestyle='None', markersize=1, linewidth=1)
            plt.plot(lower_surface_langtry[:, 3], var_data_langtry_lower, label='Langtry Menter SST (Lower Surface)', color='r', marker='x', linestyle='None', markersize=1, linewidth=1)
...
for z_value in next(iter(all_case_data.values()))["menter"].keys():
    ...
    for case, data in all_case_data.items():
        ...
        alpha = float(case.split('_')[2][5:])
        case_label = f'M{mach:.2f}_Re{int(reynolds)}_alpha{alpha:.0f}'
        for model_key, model_label in zip(["menter", "langtry"], ["Menter SST", "Langtry Menter SST"]):
